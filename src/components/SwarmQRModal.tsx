...
export const SwarmQRModal: React.FC<SwarmQRModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-md bg-[#111219] border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
          ...
          <button
            ...
            className="w-8 h-8 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white flex items-center justify-center border border-zinc-800 transition-colors cursor-pointer"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        {/* Tab Switcher */}
        <div className="grid grid-cols-2 p-1.5 bg-zinc-900/90 border-b border-zinc-800/80 text-xs font-medium">
          <button
            onClick={() => {
              stopCamera();
              setActiveTab('my_qr');
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          {activeTab === 'my_qr' ? (
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div className="p-3 bg-[#111219] rounded-2xl border-2 border-zinc-800 shadow-xl relative group">
                {qrDataUrl ? (
                  <img
                    src={qrDataUrl}
                    alt="My Safety Verification QR Code"
                    className="w-56 h-56 sm:w-64 sm:h-64 rounded-xl object-contain bg-[#111219]"
                  />
                ) : (
                ...
                {/* Center Shield Icon */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-700 flex items-center justify-center shadow-lg pointer-events-none">
              ...
              <div className="space-y-1">
                ...
                <div className="font-mono text-[11px] text-amber-300/90 break-all px-2 py-0.5 rounded bg-zinc-900 border border-zinc-800 inline-block">
                  {currentUser.shortKey}
                </div>
              </div>

              {/* Reliability Score Card */}
              <div className="w-full p-3 rounded-xl bg-zinc-900/80 border border-zinc-800 flex items-center justify-between text-left">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-xl bg-emerald-950/50 border border-emerald-500/40 flex items-center justify-center text-emerald-400">
                    <Award className="w-4 h-4" />
                  </div>
                  <div>
                    <div className="text-xs font-semibold text-white flex items-center gap-1.5">
                      <span>Trust & Safety Score: {currentUser.reliabilityScore}/100</span>
                    </div>
                ...
                <div className="text-right">
                  <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-emerald-950/60 text-emerald-300 border border-emerald-700/50 font-semibold">
                    Verified Member
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 w-full pt-1">
                <button
                  ...
                  className="flex-1 min-h-[40px] px-3 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-xs font-medium text-zinc-200 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                >
                  {isCopied ? (
                    <>
                ...
                <button
                  ...
                  className="min-h-[40px] px-3 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-xs font-medium text-zinc-300 hover:text-white transition-colors flex items-center gap-1.5 cursor-pointer"
              ...
              <div className="text-[11px] text-zinc-500 leading-normal text-center max-w-xs">
                Have your meetup partner scan this code with their camera to verify each other in person and boost mutual trust scores.
              </div>
            ...
            <div className="space-y-4">
              ...
              {scannedPeerPayload ? (
                <div className="p-4 rounded-2xl bg-zinc-900 border border-amber-500/40 space-y-3.5 animate-in fade-in">
                  <div className="flex items-center justify-between pb-2 border-b border-zinc-800">
                    ...
                    <span className="text-[10px] font-mono text-emerald-400 bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-800/40">Secure & Verified</span>
                  </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-black/40 border border-zinc-800 text-[11px] space-y-1">
                    <div className="text-zinc-500 text-[10px] uppercase font-mono">Safety Match Code</div>
                    <div className="font-mono text-amber-300 font-bold tracking-wider">
                      {scannedPeerPayload.fingerprint?.toUpperCase()}
                    </div>
                  </div>

                  {/* Benefit explanation */}
                  <div className="p-2.5 rounded-xl bg-emerald-950/30 border border-emerald-600/30 text-[11px] text-emerald-300 flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className="min-h-[40px] px-3 py-2 text-xs font-medium text-zinc-400 hover:text-white bg-zinc-800 rounded-xl transition-colors cursor-pointer"
                    >
                      Cancel
                    </button>
                    ...
                    <button
                      ...
                      className={`flex-1 min-h-[40px] px-4 py-2 text-xs font-semibold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-md ${
                        verificationSuccess
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative w-full h-48 bg-zinc-950 rounded-2xl border border-zinc-800 overflow-hidden flex flex-col items-center justify-center">
                    {cameraActive ? (
                      <>
                        <video
                          ref={videoRef}
                        ...
                        <button
                          onClick={stopCamera}
                          className="absolute bottom-2.5 px-3 py-1 text-[11px] font-medium bg-black/70 hover:bg-black text-white rounded-lg backdrop-blur-md border border-white/10 cursor-pointer"
                        >
                          Stop Camera
                        </button>
                      </>
                      ...
                      <div className="text-center p-4 space-y-2">
                        ...
                        <button
                          onClick={startCamera}
                          className="min-h-[36px] px-4 py-1.5 text-xs font-semibold text-black bg-amber-400 hover:bg-amber-300 rounded-xl transition-colors cursor-pointer inline-flex items-center gap-1.5"
                        >
                          <Camera className="w-3.5 h-3.5" />
                          <span>Launch Camera Scanner</span>
                        </button>
                        {cameraError && (
                          <p className="text-[11px] text-amber-300/80 max-w-xs">{cameraError}</p>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs text-zinc-400">
                      ...
                      <span className="text-[11px] text-zinc-500">Tap to simulate scan</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {datingProfiles.slice(0, 4).map((peer) => (
                        <button
                          ...
                          className="p-2.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 text-left transition-all cursor-pointer flex items-center justify-between group"
                        >
                  ...
                  <div className="p-3 rounded-xl bg-zinc-900/60 border border-zinc-800 space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-1.5 text-xs text-white placeholder-zinc-600 focus:outline-none focus:border-amber-400 font-mono"
                      />
                      <button
                        onClick={handleParseManualInput}
                        className="min-h-[34px] px-3 py-1.5 text-xs font-semibold text-zinc-200 bg-zinc-800 hover:bg-zinc-700 rounded-xl transition-colors cursor-pointer"
                      >
                        Verify
                      </button>
                    </div>
