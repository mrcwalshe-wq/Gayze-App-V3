...
export const ChatRoomView: React.FC<ChatRoomViewProps> = ({
  ...
  return (
    <div className="flex h-[calc(100dvh-175px)] sm:h-[calc(100vh-140px)] min-h-[460px] bg-[#0c0d12] rounded-2xl border border-zinc-800/80 overflow-hidden shadow-xl">
      {/* Sidebar: Chats & Conversations */}
      <div 
        className={`w-full sm:w-72 md:w-80 bg-[#0f1016] border-r border-zinc-800/80 flex flex-col shrink-0 ${
      ...
      >
        ...
        <div className="p-3.5 border-b border-zinc-800/80 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-zinc-200 tracking-wide uppercase font-sans">
              Encrypted Chats ({rooms.length})
            </span>
          </div>
          <span className="text-[10px] text-zinc-400 bg-zinc-800 px-2 py-0.5 rounded border border-zinc-700/60 font-mono">
            E2EE
          </span>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3.5 transition-colors flex items-start gap-3 cursor-pointer min-h-[56px] ${
                  isSelected
                    ? 'bg-zinc-800/70 border-l-2 border-amber-400'
        ...
        {/* Footnote */}
        <div className="p-3 bg-[#0a0b0f] border-t border-zinc-800/80 text-[11px] text-zinc-500 flex items-center justify-between">
          <span>AES-256-GCM</span>
          <span className="text-zinc-400">Direct peer storage</span>
        </div>
      </div>
      ...
      {currentRoom ? (
        <div 
          className={`flex-1 flex flex-col bg-[#0a0b10] min-w-0 ${
            mobileView === 'list' ? 'hidden sm:flex' : 'flex'
          }`}
        >
          {/* Header */}
          <div className="h-14 px-3 sm:px-4 border-b border-zinc-800/80 flex items-center justify-between gap-2 bg-[#0e0f16]">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              {/* Mobile Back Button */}
              <button
                onClick={() => setMobileView('list')}
                className="sm:hidden min-h-[38px] min-w-[38px] -ml-1 flex items-center justify-center text-zinc-400 hover:text-white rounded-lg hover:bg-zinc-800 transition-colors"
                aria-label="Back to rooms"
              >
                <ChevronLeft className="w-5 h-5" />
              </button>

              <div className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700/80 flex items-center justify-center text-xs font-semibold text-amber-300 shrink-0">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              ...
              {currentRoom.type === 'direct' && (
                currentRoom.verifiedViaQR ? (
                  <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-950/50 border border-emerald-500/40 text-[11px] text-emerald-300 font-medium">
                    <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
                  ...
                  onOpenQR && (
                    <button
                      ...
                      className="min-h-[36px] flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium text-amber-300 bg-amber-950/40 hover:bg-amber-900/50 border border-amber-600/40 rounded-lg transition-colors cursor-pointer"
                      title="Verify public keys in person via QR code"
                    >
                      <QrCode className="w-3.5 h-3.5 text-amber-400" />
                      <span className="hidden sm:inline">Verify via QR</span>
                    </button>
                  )
                )
              )}
              ...
              <button
                ...
                className={`min-h-[36px] flex items-center gap-1.5 px-2 sm:px-2.5 py-1 text-[11px] rounded-lg border transition-colors cursor-pointer ${
              ...
              <button
                ...
                className="min-h-[36px] min-w-[36px] flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium text-zinc-300 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700/60 rounded-lg transition-colors cursor-pointer"
              >
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
          ...
          <div className="px-3 sm:px-4 py-2 bg-zinc-900/60 border-b border-zinc-800/80 flex items-center justify-between text-[11px] text-zinc-400">
            <div className="flex items-center gap-2">
              <Lock className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <span className="truncate">End-to-End Encrypted · Messages stored only on your device</span>
            </div>
            ...
            {currentMessages.map((msg) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`max-w-[88%] sm:max-w-md rounded-2xl px-3.5 py-2 text-xs sm:text-sm leading-relaxed ${
                      isMe
                        ? 'bg-amber-400 text-black font-medium rounded-tr-sm'
                        : 'bg-[#161822] text-zinc-100 border border-zinc-800 rounded-tl-sm'
                    }`}
                  >
                    ...
                    <div className="mt-1 flex items-center justify-end gap-1.5 pt-0.5 text-[10px] opacity-75">
                      ...
                      >
                        {isInspecting ? 'Hide Security Details' : 'Security Details'}
                      </button>
                    </div>
                  </div>
                  ...
                  {isInspecting && (
                    <div className="mt-1.5 p-2.5 bg-zinc-900 border border-zinc-700 rounded-xl text-[10px] text-zinc-300 max-w-md space-y-1">
                      <div className="text-amber-400 font-semibold flex items-center gap-1.5">
                        <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
                        <span>Message Encryption Details</span>
                      </div>
                      <div className="text-zinc-400">Encryption standard: AES-256 (end-to-end)</div>
                      <div className="text-zinc-400 truncate font-mono">Verification Code: 0x{msg.nonceHex}</div>
          ...
          {/* Message Input Box */}
          <form onSubmit={handleSend} className="p-2.5 sm:p-3 bg-[#0e0f16] border-t border-zinc-800/80">
            <div className="flex items-center gap-2 bg-[#141620] border border-zinc-700/80 rounded-xl px-3 py-1.5 focus-within:border-amber-400 transition-colors">
              <input
                type="text"
                value={inputText}
              ...
              <button
                ...
                className="w-8 h-8 rounded-lg bg-amber-400 hover:bg-amber-300 disabled:opacity-30 disabled:hover:bg-amber-400 text-black flex items-center justify-center shrink-0 transition-colors cursor-pointer"
      ...
      {isSafetyModalOpen && currentRoom && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="relative w-full max-w-md bg-[#12131a] border border-zinc-800 rounded-2xl p-5 sm:p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              Compare this security code with <strong className="text-white">{currentRoom.name}</strong> or scan each other's QR code in person to guarantee your chat is direct and private.
            </p>
            ...
            <div className="p-4 bg-zinc-900 border border-zinc-800 rounded-xl space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 font-mono text-center text-sm font-bold text-emerald-400">
                <span className="p-2 bg-zinc-950/80 rounded border border-zinc-800">48291</span>
                <span className="p-2 bg-zinc-950/80 rounded border border-zinc-800">90312</span>
                <span className="p-2 bg-zinc-950/80 rounded border border-zinc-800">65120</span>
                <span className="p-2 bg-zinc-950/80 rounded border border-zinc-800">11894</span>
                <span className="p-2 bg-zinc-950/80 rounded border border-zinc-800">77239</span>
                <span className="p-2 bg-zinc-950/80 rounded border border-zinc-800">55401</span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5 pt-2">
              {onOpenQR && currentRoom.type === 'direct' && (
                <button
                  ...
                  className="w-full sm:flex-1 py-2.5 text-xs font-semibold text-amber-300 bg-amber-950/60 hover:bg-amber-900/70 border border-amber-600/40 rounded-xl transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                >
                  <QrCode className="w-4 h-4 text-amber-400" />
                  <span>Scan / Show QR Key</span>
                </button>
              )}
              ...
              <button
                ...
                className="w-full sm:flex-1 py-2.5 text-xs font-semibold text-black bg-emerald-400 hover:bg-emerald-300 rounded-xl transition-colors cursor-pointer flex items-center justify-center gap-1.5"
              >
